...
def extract():
    ...
    if time_now >= time(4) and time_now < time(9):
        try:
            ...
            s = Session()
            # execute query
            src_tables = s.execute(""" SELECT table_name FROM information_schema.tables WHERE table_schema = 'smc' and table_name in ('asset','asset_ip_info','customer','bb_instance','bb_device','bb_history','circuit','circuit_protocol_type','circuit_status','circuit_transport_type','location','sf_guard_user','sales_account_manager') """)
    ...
    #power_utilizations
    if time_now >= time(9) and time_now <= time(10):
        try:
            engine = mysql_aux1
            Session = scoped_session(sessionmaker(bind=engine))
            s = Session()
