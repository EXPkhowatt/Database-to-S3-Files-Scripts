...
cnxn_acm=mysql.connector.connect(
    host=os.environ['DB_HOST_acm'],
    user=os.environ['DB_USER_acm'],
    passwd=os.environ['DB_PWD_acm'],
    database=os.environ['DB_NAME_acm']
)

mysql_acm = create_engine(os.environ['ENG_acm'])
mysql_acm2 = create_engine(os.environ['ENG_acm2'])

cnxn_810=mysql.connector.connect(
    host=os.environ['DB_HOST_810'],
    user=os.environ['DB_USER_810'],
    passwd=os.environ['DB_PWD_810'],
    database=os.environ['DB_NAME_810']
)
...
def extract():
    ...
    try:
        ...
        s = Session()
        # execute query
        src_tables = s.execute(""" SELECT table_name FROM information_schema.tables WHERE table_schema = 'smc' and table_name in ('asset_ip_info','customer') """)
    ...
    try:
        ...
        for tbl in src_tables:
            # query and load save data to dataframe
            df = pd.read_sql_query(f""" select id, vsaname, ipaddress, replace(customer_name,',', '') as customer_name, replace(report_name,',', '') as report_name,
                location, smc_asset_id, brm_acct, commit_quota, overage_charge, active, sslvpn_burst_id FROM {tbl[0]}""", engine)
            load(df, tbl[0])
    except Exception as e:
